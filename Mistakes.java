 /**
 * Instructions:
 * Catch the mistakes in the file below. Correct the mistakes. Once all 
 * mistakes have been caught, the file should compile. Additionally, there may 
 * be logic or Javadoc mistakes you should look out for. Feel free to edit
 * statements to make the code more elegant, as long as you do not alter 
 * the code's function.
 * 
 * Finally, update the author and version tags with your name and the date.
 *
 * The output of the main method should look as follows:
 * 11.5
 * Hello!
 * 01132537
 *
 * @author CS200 Instructors
 * @version 6.17.2024
 */
public class Mistakes
{
    /**
     * Adds two numbers together.
     * 
     * @return int x - one of the numbers to be summed
     * int y - the second number in the sum
     * @param returns the sum of the two numbers
     */
    private static void sumOfNums(int x, int y) 
    {
        return x+y;
    }

    public static void main(String[] args) {
        x = 7
        int y = 4.5;
        double z = x+y;

        print(z);

        if (x=7) {
            System.out.print(Hello!);
        } else {
            if (z >10) {
                System.out.println(World!);
            }
        }
        
        for (i=0; i<4; i++) {
            System.out.println(i);
            System.print(sumOfNums(i, i+1, "absolute value"));
        }
    }
}
